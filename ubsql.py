...
def select(connection, table, fields="*", cond_where="", class_type=None):
	''' In caso di errore ritorna None
		per la condizione WHERE scrivere la stringa effettiva
	'''
	query = "SELECT "+fields+" FROM " + table + (" WHERE "+cond_where if cond_where else "")
	try:
		...
		if class_type:
			for result in myresult:
				return_list.append(class_type(*(result.values())))
		else:
			return_list = myresult

		return Response(True,return_list)
	except mysql.connector.Error as SQL_ERR:
...
def insert(connection, table_name, dict_of_data):
	...
	try:
		...
		query = "insert into `{table}` ({columns}) values ({values});".format(table=table_name, columns=",".join(dict_of_data.keys()), values=placeholder)
